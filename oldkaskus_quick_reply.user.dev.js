...
// -!--latestupdate
//
// v3.2.9 - 2012-08-25 . 1345913398412
//   fix bbcode "maho[s]" to "genit[s]"
//   deprecated uploader [lulzimg, imagedum]
...
/*
//
// v3.2.8 - 2012-05-27 . 1338128688850
...
gvar.sversion = 'v' + '3.2.9b';
gvar.scriptMeta = {
  timestamp: 1345913398412 // version.timestamp

 ,dtversion: 120825329 // version.date
  ...
  */
  ...
  switch(loc){
    ...
    CSS=''
      ...
      +'center{display:none'+i+'}'
      +'p:first-child{position:absolute;margin-top:-15px'+i+'}'
...
function appendAvatar(){
    ...
    if(isDefined(gvar.user.id)){
       ...
       _o('click',$D('#refetch'),function(){
         ...
         el=createEl('a',{href:gvar.domain + 'member.php?u='+gvar.user.id,'class':'cleanlink'},'<b><small>'+gvar.user.name+'</small>'+(gvar.user.isDonatur ? ' <span style="color:red;">[$]</span>':'')+'</b>');
...
var Avatar = {
  ...
  init:function(user){
    Avatar.user = user;
    Avatar.uri=gvar.domain + '/member.php?u='+Avatar.user.id;
  },
...
var QRdp = {
 ...
 ,updLast:function(x){ cK.s(QRdp.key, String(x), '/', gvar.domain.replace(/http\:\/\/|\//gi,'')); QRdp._updLast( String(x) )}
...
var ST = {
 ...
 ,reset_setting: function(){
    var home=[gvar.domain + 'showthread.php?t=3170414','http:/'+'/userscripts.org/topics/58227'];
    var space = '';
...
,[H+'fd_2.gif', ':cd:', 'Cape deeehh']
 ...
 ,getBOT_greet: function(min, max){
    ...
    cK.s('last_greet', String(lastgreet), '/', gvar.domain.replace(/http\:\/\/|\//gi,''));
    return c[lastgreet];
  }
 ...
 ,getTPL_Settings_About: function(){
    ...
    return (''
     ...
     +'<div style="height:7px;"></div>'
     +'KASKUS brand is a registered trademark of '+ gvar.domain.replace(/http\:\/\/|\//gi,'') +'<br>'
     +gvar.fullname+' (QR) is not related to or endorsed by '+ gvar.domain.replace(/http\:\/\/|\//gi,'') +' in any way.<br>'
